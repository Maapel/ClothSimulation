...
public class RopeWindingController : MonoBehaviour
{
            ...
            foreach (var hitObject in hits)
            {
                ...
                if (hitObject.transform != transform)
                {
                    ...
                    if (hitObject.Raycast(ray, out hit, 3f))
                    {
                        ...
                        {
                            if (((v1.pos) + (1 + r) * Vector3.Dot((collisionPointIdeal - (v1.pos)), normal) * normal - v1.prevpos).magnitude < r / 5f)
                            {
                            ...
                            {
                                v1.pos = collisionPointIdeal;
                                v1.prevpos = collisionPointIdeal;
                            }
                        }
                        else
                        {
